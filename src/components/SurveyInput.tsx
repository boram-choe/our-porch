  ...
  return (
    <div className="space-y-2 md:space-y-4">
      <div className="flex items-center justify-between">
        <label className="flex items-center gap-2 text-xs md:text-sm font-black text-slate-400 uppercase tracking-widest">
          <Icon size={16} className="md:w-[18px] md:h-[18px]" /> {label}
        </label>
  ...
  return (
    <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 overflow-y-auto bg-slate-950/80 backdrop-blur-md">
      <motion.div 
        ...
        className="bg-white w-full max-w-xl rounded-2xl md:rounded-[3.5rem] shadow-2xl relative overflow-hidden flex flex-col max-h-[95vh] md:max-h-[90vh]"
      >
        {/* Header */}
          ...
          <div className="flex items-center gap-3 md:gap-6 mb-2 md:mb-3">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="text-lg md:text-3xl font-black text-slate-950 tracking-tighter leading-none">툇마루단 현장 조사 ✨</h2>
              ...
              <p className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest mt-1 md:mt-2">{formData.address || "신규 공실 등록"}</p>
            </div>
          </div>
          <button onClick={onClose} className="absolute top-6 md:top-12 right-6 md:right-12 text-slate-300 hover:text-slate-600 transition-colors">
            <X size={20} className="md:w-8 md:h-8" />
          </button>
        </div>

        {/* Content */}
        <div className="flex-1 overflow-y-auto p-6 md:p-12 space-y-6 md:space-y-12 custom-scroll no-scrollbar">
          {/* 이미지 섹션 */}
          <div className="space-y-4 md:space-y-6">
            <label className="flex items-center gap-2 text-xs md:text-sm font-black text-slate-400 uppercase tracking-widest">
              <Camera size={16} className="md:w-[18px] md:h-[18px]" /> 현장 사진 (최대 5장)
            </label>
                  ...
                  {formData.images?.[idx] ? (
                    <>
                      ...
                      <div className="absolute inset-0 bg-slate-950/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <button onClick={() => removeImage(idx)} className="bg-red-500 text-white p-1.5 md:p-3 rounded-lg md:rounded-xl shadow-xl hover:scale-110 transition-all">
            ...
            <p className="text-[10px] md:text-xs font-bold text-slate-400 text-center italic mt-2 md:mt-3">첫 번째 사진이 투표 화면 배경으로 사용됩니다.</p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 mb-2">
            <div className="p-4 md:p-8 bg-amber-50 rounded-2xl md:rounded-[2.5rem] border-2 border-amber-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3 md:gap-4">
                ...
                <div>
                  <p className="text-[9px] md:text-[10px] font-black text-amber-600 uppercase tracking-widest">핀 위치 수정</p>
                  <p className="text-xs md:text-sm font-black text-slate-900 mt-0.5">{formData.lat?.toFixed(6)}, {formData.lng?.toFixed(6)}</p>
                </div>
              </div>
              <button 
                onClick={onEditLocation}
                className="w-full sm:w-auto px-5 py-2.5 bg-slate-950 text-white text-[10px] md:text-[11px] font-black rounded-xl hover:bg-slate-800 active:scale-95 transition-all shadow-lg text-center"
              >
                지도에서 위치 수정하기 →
              </button>
            </div>
          ...
          <div className="space-y-4 md:space-y-6">
            <label className="flex items-center gap-2 text-xs md:text-sm font-black text-slate-400 uppercase tracking-widest">
              <Clock size={16} className="md:w-[18px] md:h-[18px]" /> 공실 기간 (현장 확인 필요)
            </label>
          ...
          <div className="space-y-4 md:space-y-8">
            <div className="flex items-center gap-3 md:gap-4 mb-2 md:mb-4">
              <div className="h-6 w-1.5 md:h-10 md:w-2 bg-blue-500 rounded-full" />
              <h3 className="text-lg md:text-2xl font-black text-slate-950 tracking-tight">임대 조건</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
          ...
          <div className="space-y-4 md:space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
              {[
                ...
                <button 
                  ...
                  className={`py-3 md:py-5 rounded-xl md:rounded-2xl flex flex-col items-center gap-1 md:gap-2 border-2 transition-all ${formData.status === s.id ? 'bg-slate-950 border-slate-950 text-white shadow-xl scale-105' : 'bg-slate-50 border-slate-50 text-slate-400 hover:border-slate-200'}`}
                >
                  <span className="text-lg md:text-xl">{s.icon}</span>
                  <span className="text-[11px] md:text-[12px] font-black">{s.label}</span>
                </button>
              ))}
            </div>
            ...
            <AnimatePresence mode="wait">
              {formData.status === 'merged' && (
                ...
                >
                  <div className="p-4 md:p-8 bg-purple-50 rounded-2xl md:rounded-[2.5rem] border-2 border-purple-100 space-y-4 md:space-y-6">
                    {nearestVacancy && (
                      <div className="bg-white p-4 md:p-6 rounded-xl md:rounded-2xl border border-purple-100 shadow-sm space-y-2">
                        <div className="flex items-center justify-between">
                    ...
                    <div className="space-y-2 md:space-y-4">
                      ...
                      <input
                        ...
                        className="w-full bg-white border-2 border-purple-100 rounded-xl md:rounded-2xl py-3 px-4 md:py-4 md:px-6 font-bold text-slate-950 focus:outline-none focus:border-purple-500 transition-all text-xs md:text-sm"
                      />
              ...
              {formData.status === 'rejected' && (
                <motion.div 
                  ...
                  className="space-y-6 overflow-hidden"
                >
                  <div className="p-4 md:p-8 bg-rose-50 rounded-2xl md:rounded-[2.5rem] border-2 border-rose-100 space-y-4 md:space-y-6">
                    <div className="space-y-2 md:space-y-4">
          ...
          <div className="space-y-4 md:space-y-8">
            ...
            <div className="space-y-3 md:space-y-4 font-bold text-slate-800 leading-relaxed italic">
              ...
              <label className="flex items-center gap-2 text-xs md:text-sm font-black text-slate-400 uppercase tracking-widest">
                <MessageSquare size={16} className="md:w-[18px] md:h-[18px]" /> 한줄평 (비고)
              </label>
              <textarea
          ...
          <div className="space-y-4 md:space-y-8">
            <div className="flex items-center gap-3 md:gap-4 mb-2 md:mb-4">
              <div className="h-6 w-1.5 md:h-10 md:w-2 bg-teal-500 rounded-full" />
              <h3 className="text-lg md:text-2xl font-black text-slate-950 tracking-tight">부동산 정보</h3>
            </div>
        ...
        {initialData?.updatedAt && (
          <div className="px-6 md:px-12 py-3 md:py-6 bg-slate-50 border-y border-slate-100">
        ...
        {/* Footer */}
        <div className="p-6 md:p-12 pt-4 md:pt-6 border-t border-slate-50 flex-shrink-0">
          <button 
            onClick={handleSave}
            className="w-full py-4 md:py-6 bg-slate-950 text-white rounded-2xl md:rounded-[2.5rem] text-base md:text-xl font-black shadow-2xl hover:bg-slate-800 active:scale-95 transition-all flex items-center justify-center gap-2 md:gap-4"
          >
            조사 데이터 저장하기 <Check className="w-5 h-5 md:w-8 md:h-8" />
          </button>
        </div>
      </motion.div>
    </div>
